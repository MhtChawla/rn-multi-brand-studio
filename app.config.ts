...
export default {
  ...
  extra: {
    brand,
    // Embed icon.png as a base64 data URI so the Home screen can display it
    // without dynamic require() (which Metro cannot resolve at runtime).
    // Every brand generated by phase-6 has assets/icon.png.
    logoPng: (() => {
      const iconPath = path.join(brandDir, 'assets', 'icon.png');
